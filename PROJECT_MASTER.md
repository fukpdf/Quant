...
- 15 DB tables, 8 core services (metrics-collector, service-health-engine, scheduler-monitor, strategy-health-engine, ai-health-engine, execution-health-engine, alert-engine, incident-manager), ops-scheduler with 10 background loops, 13 route files, 29 endpoints under `/api/v1/ops/*`, OpenAPI 0.12.0 with `operations` tag, codegen regenerated. **READ-ONLY platform intelligence — no trades, orders, or positions are affected.**
